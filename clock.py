...
def scheduled_job():
    ...
    with smart_run(session):
        ...
        #session.follow_likers(follow_likers_of_users, photos_grab_amount=1, follow_likers_per_photo=14, randomize=False, sleep_delay=60, interact=False)
        session.unfollow_users(amount=int(os.environ['unfollow_amount']), allFollowing=True, style="LIFO", unfollow_after=48 * 60 * 60, sleep_delay=random.randint(1, 10)*60)
        # activity
